...
import os
import time
from .io_handler import perform_rust_computation
from .write_format import write_formated
...
class RustExperiment:
    ...
    def write_parameter_file(self, L: int, rounding = 3) -> None:
        ...
        try:
            write_formated(self.param_files[L], self.out_files[L], L, L, self.therm_steps[L], self.measure_steps[L], self.temperatures, rounding)
        ...
        for L in self.lengths:
            if self.has_output(L):
                ...
                if self.temperatures is None:
                    ...
                    self.set_temperatures(results[L].temperatures)
                    # temp_set = True
        # if temp_set:
    ...
    @staticmethod
    def load_results(name: str, folder: str, lengths) -> tuple[np.ndarray, dict[int, RustMonteCarloData]]:
        exp     = RustExperiment(name, folder, lengths)
        results = exp.get_results()
        if results is None:
            print(">> Failed loading results")
            return None
        return (exp.temperatures, results)
